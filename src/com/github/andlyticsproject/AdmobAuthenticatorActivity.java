...
private void initLayout() {
	...
	if (mUsername == null) {
		mUsername = DeveloperAccountManager.getInstance(this)
...
public void handleLogin(final View v) {
	...
	if (TextUtils.isEmpty(mUsername) || TextUtils.isEmpty(mPassword)) {
		...
		showProgress();
		mAuthThread = AdmobAuthenticationUtilities.attemptAuth(
			mUsername, mPassword, mHandler, AdmobAuthenticatorActivity.this);
	}
}

private void finishConfirmCredentials(final boolean result) {
	Account account =
		new Account(mUsername, AdmobAccountAuthenticator.ACCOUNT_TYPE_ADMOB);
...
private void finishLogin() {
	...
	if (mAuthtokenType != null &&
	    mAuthtokenType.equals(AdmobAccountAuthenticator.AUTHTOKEN_TYPE_ADMOB)) {
...
public void onAuthenticationResult(final String result) {
	...
	if ("true".equalsIgnoreCase(result)) {
		...
		if (AdmobRequest.ERROR_NETWORK_ERROR.equals(result)) {
			Toast
			.makeText(AdmobAuthenticatorActivity.this,
		...
		if (AdmobRequest.ERROR_REQUEST_INVALID.equals(result)) {
			Toast
			.makeText(AdmobAuthenticatorActivity.this,
			          getString(R.string.admob_linked_accounts_error),
			          Toast.LENGTH_LONG)
			.show();
		}
