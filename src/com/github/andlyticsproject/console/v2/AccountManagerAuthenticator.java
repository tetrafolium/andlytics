...
private static final String TAG =
	AccountManagerAuthenticator.class.getSimpleName();
...
private static final Uri TOKEN_AUTH_URL =
	Uri.parse("https://www.google.com/accounts/TokenAuth");
...
public AccountManagerAuthenticator(final String accountName,
	...
	try {
		...
		Bundle authResult =
			...
			.getAuthToken(account,
			              "weblogin:service=androiddeveloper&continue=" +
		...
		if (authResult.containsKey(AccountManager.KEY_INTENT)) {
			...
			if (activity == null) {
				...
				PendingIntent contentIntent =
					PendingIntent.getActivity(ctx, accountName.hashCode(), authIntent,
		...
		if (!webloginUrl.contains("MergeSession") ||
		    webloginUrl.contains("WILL_NOT_SIGN_IN")) {
			Log.d(TAG, "Most probably additional verification is required, "
			      + "opening browser");

			String sid =
				accountManager
				.getAuthToken(account, "SID", null, activity, null, null)
				.getResult()
				.getString(AccountManager.KEY_AUTHTOKEN);
			if (sid == null) {
				throw new AuthenticationException(
					      "Authentication error: cannot get SID.");
			}
			String lsid =
				accountManager
				.getAuthToken(account, "LSID", null, activity, null, null)
				.getResult()
				.getString(AccountManager.KEY_AUTHTOKEN);
			if (lsid == null) {
				throw new AuthenticationException(
					      "Authentication error: cannot get LSID.");
			}
