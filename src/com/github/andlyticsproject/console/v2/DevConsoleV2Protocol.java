...
public class DevConsoleV2Protocol {
  ...
  // Base urls
  static final String URL_DEVELOPER_CONSOLE =
      "https://play.google.com/apps/publish";
  static final String URL_APPS =
      DevConsoleV2Protocol.URL_DEVELOPER_CONSOLE + "/androidapps";
  static final String URL_STATISTICS =
      DevConsoleV2Protocol.URL_DEVELOPER_CONSOLE + "/statistics";
  static final String URL_REVIEWS =
      DevConsoleV2Protocol.URL_DEVELOPER_CONSOLE + "/reviews";

  // Templates for payloads used in POST requests
  static final String FETCH_APPS_TEMPLATE =
      "{\"method\":\"fetch\","
      + "\"params\":{\"2\":1,\"3\":7},\"xsrf\":\"%s\"}";
  // 1$: comma separated list of package names
  static final String FETCH_APPS_BY_PACKAGES_TEMPLATE =
      "{\"method\":\"fetch\","
      + "\"params\":{\"1\":[%1$s],\"3\":1},\"xsrf\":\"%2$s\"}";
  // 1$: package name, 2$: XSRF
  static final String FETCH_APP_TEMPLATE =
      "{\"method\":\"fetch\","
      + "\"params\":{\"1\":[\"%1$s\"],\"3\":0},\"xsrf\":\"%2$s\"}";
  // 1$: package name, 2$: XSRF
  static final String GET_RATINGS_TEMPLATE =
      "{\"method\":\"getRatings\","
      + "\"params\":{\"1\":[\"%1$s\"]},\"xsrf\":\"%2$s\"}";
  // 1$: package name, 2$: start, 3$: num comments to fetch, 4$: display locale,
  // 5$ XSRF
  static final String GET_REVIEWS_TEMPLATE =
      "{\"method\":\"getReviews\","
      +
      "\"params\":{\"1\":\"%1$s\",\"2\":%2$d,\"3\":%3$d,\"8\":\"%4$s\",\"10\":0,\"18\":1},\"xsrf\":\"%5$s\"}";
  // 1$: package name, 2$: stats type, 3$: stats by, 4$: XSRF
  static final String GET_COMBINED_STATS_TEMPLATE =
      "{\"method\":\"getCombinedStats\","
      +
      "\"params\":{\"1\":\"%1$s\",\"2\":1,\"3\":%2$d,\"4\":[%3$d]},\"xsrf\":\"%4$s\"}";
  // %1$s: package name, %2$s: XSRF
  static final String REVENUE_SUMMARY_TEMPLATE =
      "{\"method\":\"revenueSummary\",\"params\":{\"1\":\"%1$s\",\"2\":\"\"},\"xsrf\":\"%2$s\"}";
  // %1$s: package name, %2$s: XSRF
  static final String REVENUE_HISTORICAL_DATA =
      "{\"method\":\"historicalData\",\"params\":{\"1\":\"%1$s\",\"2\":\"\"},\"xsrf\":\"%2$s\"}";
  ...
  SessionCredentials getSessionCredentials() { return sessionCredentials; }

  void setSessionCredentials(final SessionCredentials sessionCredentials) {
    this.sessionCredentials = sessionCredentials;
  }

  boolean hasSessionCredentials() { return sessionCredentials != null; }

  void invalidateSessionCredentials() { sessionCredentials = null; }
  ...
  void addHeaders(final HttpPost post, final String developerId) {
    ...
    post.addHeader("X-GWT-Module-Base",
                   "https://play.google.com/apps/publish/gwt/");
    post.addHeader("Referer", "https://play.google.com/apps/publish/?dev_acc=" +
                                  developerId);
  }
  ...
  String createFetchAppInfosRequest() {
    ...
    return String.format(FETCH_APPS_TEMPLATE,
  ...
  List<AppInfo> parseAppInfosResponse(final String json,
                                      final String accountName,
                                      final boolean skipIncomplete) {
    try {
  ...
  private static void saveDebugJson(final String json) {
    FileUtils.tryWriteToDebugDir(
        String.format("console_reply_%d.json", System.currentTimeMillis()),
        json);
  }
  ...
  String createFetchAppInfoRequest(final String packageName) {
    ...
    return String.format(FETCH_APP_TEMPLATE, packageName,
                         sessionCredentials.getXsrfToken());
  }

  String createFetchStatisticsRequest(final String packageName,
                                      final int statsType) {
    checkState();
  ...
  void parseStatisticsResponse(final String json, final AppStats stats,
                               final int statsType) {
    try {
  ...
  String createFetchRatingsRequest(final String packageName) {
    ...
    return String.format(GET_RATINGS_TEMPLATE, packageName,
                         sessionCredentials.getXsrfToken());
  }
  ...
  String createFetchCommentsRequest(final String packageName, final int start,
                                    final int pageSize,
                                    final String displayLocale) {
    checkState();

    return String.format(GET_REVIEWS_TEMPLATE, packageName, start, pageSize,
                         displayLocale, sessionCredentials.getXsrfToken());
  }

  String createReplyToCommentRequest(final String packageName,
  ...
  String createFetchRevenueSummaryRequest(final String packageName) {
    ...
    try {
      ...
      arr.put(new JSONObject().put("1", 11).put(
          "2", new JSONArray().put(sessionCredentials.getPreferredCurrency())));
      arr.put(new JSONObject().put("1", 18).put(
          "2",
  ...
  RevenueSummary parseRevenueResponse(final String json) {
    try {
      return JsonParser.parseRevenueResponse(
          json, sessionCredentials.getPreferredCurrency());
    } catch (JSONException ex) {
      saveDebugJson(json);
      throw new DevConsoleProtocolException(json, ex);
    }
