...
/**
 * This class contains static methods used to parse JSON from {@link
 * DevConsoleV2}
 *
 * See {@link
 * https://github.com/AndlyticsProject/andlytics/wiki/Developer-Console-v2} for
...
/**
 * Parses the supplied JSON string and adds the extracted ratings to the
 * supplied
 ...
 */
static void parseRatings(final String json, final AppStats stats)
throws JSONException {
...
/**
 * Parses the supplied JSON string and adds the extracted statistics to the
 * supplied
 * {@link AppStats} object
 ...
 * @throws JSONException
 */
static void parseStatistics(final String json, final AppStats stats,
                            final int statsType) throws JSONException {
	// Extract the top level values array
	JSONObject values =
		new JSONObject(json).getJSONObject("result").getJSONObject("1");
	/*
	 * null
	 ...
	 * null
	 * null
	 * Nested arrays containing summary and historical data broken down by
	 * dimension e.g. Android version null null App name
	 */
	// For now we just care about todays value, later we may delve into the
	// historical and dimensioned data
	JSONArray historicalData = values.getJSONObject("1").getJSONArray("1");
	JSONObject latestData =
		historicalData.getJSONObject(historicalData.length() - 1);
	/*
	 * null
	 * Date
	 * [null, value]
 ...
 * @throws JSONException
 */
static List<AppInfo> parseAppInfos(final String json,
...
throws JSONException {
	...
	for (int i = 0; i < numberOfApps; i++) {
		...
		if (packageName == null || (packageName.startsWith("tmp.") &&
			...
			Log.d(TAG, String.format("Skipping draft app %d, package name=%s", i,
			                         packageName));
			continue;
			// Draft app
		}
		...
		Log.d(TAG,
		      String.format("%s: publishState=%d", packageName, publishState));
		if (publishState != 1) {
		...
		if (!jsonApp.has("6")) {
			if (skipIncomplete) {
				Log.d(
					TAG,
					String.format(
						"Skipping app %d because no app details found: package name=%s",
						i, packageName));
			} else {
		...
		AppDetails details =
			new AppDetails(description, changelog, lastPlayStoreUpdate);
		...
		if (!jsonApp.has("3")) {
			if (skipIncomplete) {
				Log.d(
					TAG,
					String.format(
						"Skipping app %d because no app stats found: package name=%s",
						i, packageName));
			} else {
				Log.d(TAG, "Adding incomplete app: " + packageName);
				apps.add(app);
...
static List<Comment> parseComments(final String json) throws JSONException {
	...
	JSONArray jsonComments =
		new JSONObject(json).getJSONObject("result").getJSONArray("1");
	...
	for (int i = 0; i < count; i++) {
		 ...
		 * [
		 * null,
		 * "Prixing fonctionne pourtant bien sur Xperia X10. Essayez de prendre un
		 * minimum de recul, au moins 20 � 30cm, �vitez les ombres et les reflets.
		 * N'h�sitez pas � nous �crire sur contact@prixing.fr pour une assistance
		 * personnalis�e."
		 * ,
		 * null,
		 * "1350393460968"
...
static Comment parseCommentReplyResponse(final String json)
throws JSONException {
...
private static DevConsoleException parseError(final JSONObject jsonObj,
...
throws JSONException {
	...
	return new DevConsoleException(
		String.format("Error %s: %s, errorCode=%s", message, data, errorCode));
}

static RevenueSummary parseRevenueResponse(final String json,
...
throws JSONException {
	...
	for (int i = 0; i < revenueArr.length(); i++) {
		...
		double value =
			revenueObj.getJSONArray("2").optJSONObject(0).optDouble("1", 0.0) /
			1000000;
	...
	return RevenueSummary.createTotal(currency, cal.getTime(), lastDay,
