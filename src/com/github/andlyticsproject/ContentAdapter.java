...
public static synchronized ContentAdapter
getInstance(final Application appCtx) {
	if (instance == null) {
...
public AdmobStatsSummary getAdmobStats(final String siteId,
		...
		if (adUnitId == null) {
			cursor = context.getContentResolver().query(
				...
				new String[] {
					...
					AdmobTable.KEY_ROWID, AdmobTable.KEY_SITE_ID,
					AdmobTable.KEY_REQUESTS, AdmobTable.KEY_HOUSEAD_REQUESTS,
...
private AdmobStats readAdmobStats(final Cursor cursor) {
	...
	admob.setSiteId(
		cursor.getString(cursor.getColumnIndex(AdmobTable.KEY_ROWID)));
	admob.setClicks(
		cursor.getInt(cursor.getColumnIndex(AdmobTable.KEY_CLICKS)));
	admob.setCpcRevenue(
		cursor.getFloat(cursor.getColumnIndex(AdmobTable.KEY_CPC_REVENUE)));
	admob.setCpmRevenue(
		cursor.getFloat(cursor.getColumnIndex(AdmobTable.KEY_CPM_REVENUE)));
	...
	admob.setExchangeDownloads(cursor.getInt(
					   cursor.getColumnIndex(AdmobTable.KEY_EXCHANGE_DOWNLOADS)));
	admob.setFillRate(
		cursor.getFloat(cursor.getColumnIndex(AdmobTable.KEY_FILL_RATE)));
	admob.setHouseAdClicks(
		cursor.getInt(cursor.getColumnIndex(AdmobTable.KEY_HOUSEAD_CLICKS)));
	admob.setHouseadFillRate(cursor.getFloat(
					 cursor.getColumnIndex(AdmobTable.KEY_HOUSEAD_FILL_RATE)));
	admob.setHouseadRequests(
		cursor.getInt(cursor.getColumnIndex(AdmobTable.KEY_HOUSEAD_REQUESTS)));
	admob.setImpressions(
		cursor.getInt(cursor.getColumnIndex(AdmobTable.KEY_IMPRESSIONS)));
	admob.setInterstitialRequests(cursor.getInt(
					      cursor.getColumnIndex(AdmobTable.KEY_INTERSTITIAL_REQUESTS)));
	admob.setOverallFillRate(cursor.getFloat(
					 cursor.getColumnIndex(AdmobTable.KEY_OVERALL_FILL_RATE)));
	admob.setRequests(
		cursor.getInt(cursor.getColumnIndex(AdmobTable.KEY_REQUESTS)));
	admob.setRevenue(
		cursor.getFloat(cursor.getColumnIndex(AdmobTable.KEY_REVENUE)));
	String dateString =
		cursor.getString(cursor.getColumnIndex(AdmobTable.KEY_DATE));
...
public void insertOrUpdateAdmobStats(final AdmobStats admob) {
	...
	long existingId =
		getAdmobStatsIdForDate(admob.getDate(), admob.getSiteId());
	if (existingId > -1) {
		// update
		context.getContentResolver().update(
			AdmobTable.CONTENT_URI, value,
...
private long getAdmobStatsIdForDate(final Date date, final String siteId) {
	...
	Cursor mCursor = context.getContentResolver().query(
		...
		AdmobTable.KEY_SITE_ID + "='" + siteId + "' and " +
		AdmobTable.KEY_DATE + " BETWEEN '" + dateFormatStart.format(date) +
		"' and '" + dateFormatEnd.format(date) + "'",
		null, null);
...
private ContentValues createAdmobContentValues(final AdmobStats admob) {
	...
	values.put(AdmobTable.KEY_INTERSTITIAL_REQUESTS,
...
public void insertOrUpdateAppStats(final AppStats appStats,
	...
	values.put(AppStatsTable.KEY_STATS_COMMENTS,
	           appStats.getNumberOfComments());
	...
	if (appStats.getVersionCode() != null) {
		values.put(AppStatsTable.KEY_STATS_VERSIONCODE,
		           appStats.getVersionCode());
	}
	if (appStats.getNumberOfErrors() != null) {
		values.put(AppStatsTable.KEY_STATS_NUM_ERRORS,
		           appStats.getNumberOfErrors());
	}
	if (appStats.getTotalRevenue() != null) {
		values.put(AppStatsTable.KEY_STATS_TOTAL_REVENUE,
		           appStats.getTotalRevenue().getAmount());
		values.put(AppStatsTable.KEY_STATS_CURRENCY,
		           appStats.getTotalRevenue().getCurrencyCode());
	}
...
private void insertOrUpdateApp(final AppInfo appInfo) {
	...
	initialValues.put(AppInfoTable.KEY_APP_PACKAGENAME,
	                  appInfo.getPackageName());
	initialValues.put(AppInfoTable.KEY_APP_ACCOUNT, appInfo.getAccount());
	initialValues.put(AppInfoTable.KEY_APP_DEVELOPER_ID,
	                  appInfo.getDeveloperId());
	initialValues.put(AppInfoTable.KEY_APP_DEVELOPER_NAME,
	                  appInfo.getDeveloperName());
	...
	initialValues.put(AppInfoTable.KEY_APP_PUBLISHSTATE,
	                  appInfo.getPublishState());
	initialValues.put(AppInfoTable.KEY_APP_CATEGORY, -1);
	initialValues.put(AppInfoTable.KEY_APP_VERSION_NAME,
	                  appInfo.getVersionName());

	Uri uri = context.getContentResolver().insert(AppInfoTable.CONTENT_URI,
	                                              initialValues);
	...
	if (revenue != null) {
		...
		values.put(RevenueSummaryTable.LAST_DAY_TOTAL,
		           revenue.getLastDay().getAmount());
		values.put(RevenueSummaryTable.LAST_7DAYS_TOTAL,
		           revenue.getLast7Days().getAmount());
		values.put(RevenueSummaryTable.LAST_30DAYS_TOTAL,
		           revenue.getLast30Days().getAmount());
		values.put(RevenueSummaryTable.OVERALL_TOTAL,
		           revenue.getOverall().getAmount());
		values.put(RevenueSummaryTable.APPINFO_ID, appInfo.getId());

		Cursor c = null;
		try {
			c = context.getContentResolver().query(
				RevenueSummaryTable.CONTENT_URI, RevenueSummaryTable.ALL_COLUMNS,
				RevenueSummaryTable.APPINFO_ID + " =?  and " +
				RevenueSummaryTable.DATE + " = ?",
				new String[] {Long.toString(appInfo.getId()),
				              Long.toString(revenue.getDate().getTime())},
				RevenueSummaryTable.DATE + " desc");
			if (!c.moveToNext()) {
				uri = context.getContentResolver().insert(
					RevenueSummaryTable.CONTENT_URI, values);
				id = Long.parseLong(uri.getPathSegments().get(1));
				revenue.setId(id);
			} else {
				long revenueId =
					c.getLong(c.getColumnIndex(RevenueSummaryTable.ROWID));
				context.getContentResolver().update(
					RevenueSummaryTable.CONTENT_URI, values,
...
public String getAppName(final String packageName) {
	...
	try {
		cursor = context.getContentResolver().query(
			AppInfoTable.CONTENT_URI, new String[] {AppInfoTable.KEY_APP_NAME},
			AppInfoTable.KEY_APP_PACKAGENAME + "='" + packageName + "'", null,
			null);

		if (cursor.moveToFirst()) {
			return cursor.getString(
				cursor.getColumnIndex(AppInfoTable.KEY_APP_NAME));
		}
...
public List<String> getPackagesForAccount(final String account) {
	...
	try {
		cursor = context.getContentResolver().query(
			AppInfoTable.UNIQUE_PACAKGES_CONTENT_URI,
...
public List<AppInfo> getAllAppsLatestStats(final String account) {
	...
	try {
		...
		while (cursor.moveToNext()) {
			...
			appInfo.setId(
				cursor.getLong(cursor.getColumnIndex(AppInfoTable.KEY_ROWID)));
			appInfo.setAccount(account);
			appInfo.setLastUpdate(Utils.parseDbDate(cursor.getString(
									cursor.getColumnIndex(AppInfoTable.KEY_APP_LASTUPDATE))));
			appInfo.setPackageName(cursor.getString(
						       cursor.getColumnIndex(AppInfoTable.KEY_APP_PACKAGENAME)));
			appInfo.setName(
				cursor.getString(cursor.getColumnIndex(AppInfoTable.KEY_APP_NAME)));
			appInfo.setGhost(cursor.getInt(cursor.getColumnIndex(
							       AppInfoTable.KEY_APP_GHOST)) == 0
			     ? false
			     : true);
			appInfo.setSkipNotification(
				cursor.getInt(cursor.getColumnIndex(
						      AppInfoTable.KEY_APP_SKIP_NOTIFICATION)) == 0
		? false
		: true);
			appInfo.setRatingDetailsExpanded(
				cursor.getInt(cursor.getColumnIndex(
						      AppInfoTable.KEY_APP_RATINGS_EXPANDED)) == 0
		? false
		: true);
			appInfo.setIconUrl(cursor.getString(
						   cursor.getColumnIndex(AppInfoTable.KEY_APP_ICONURL)));
			appInfo.setVersionName(cursor.getString(
						       cursor.getColumnIndex(AppInfoTable.KEY_APP_VERSION_NAME)));
	...
	for (AppInfo appInfo : appInfos) {
		...
		AppStatsSummary statsForApp =
			getStatsForApp(packageName, Timeframe.LAST_TWO_DAYS, false);
...
public long setGhost(final String account, final String packageName,
                     final boolean value) {
	ContentValues initialValues = new ContentValues();
	...
	if (exisitingId > -1) {
		...
		context.getContentResolver().update(
			AppInfoTable.CONTENT_URI, initialValues,
...
public long setRatingExpanded(final String account, final String packageName,
                              final boolean value) {
	ContentValues initialValues = new ContentValues();
	initialValues.put(AppInfoTable.KEY_APP_RATINGS_EXPANDED,
	                  value == true ? 1 : 0);
...
@SuppressLint("SimpleDateFormat")
public AppStatsSummary getStatsForApp(final String packageName,
                                      final Timeframe currentTimeFrame,
	...
	try {
		cursor = context.getContentResolver().query(
			...
			new String[] {
				AppStatsTable.KEY_ROWID, AppStatsTable.KEY_STATS_PACKAGENAME,
			...
			AppStatsTable.KEY_STATS_REQUESTDATE + " desc LIMIT " + limit +
			""); // sort
		// order
		// ->
		// new
		...
		if (cursor.moveToFirst()) {
			do {
				AppStats stats = new AppStats();
				stats.setActiveInstalls(cursor.getInt(
								cursor.getColumnIndex(AppStatsTable.KEY_STATS_INSTALLS)));
				stats.setTotalDownloads(cursor.getInt(
								cursor.getColumnIndex(AppStatsTable.KEY_STATS_DOWNLOADS)));

				String dateString = cursor.getString(
					cursor.getColumnIndex(AppStatsTable.KEY_STATS_REQUESTDATE));
				stats.setDate(
					Utils.parseDbDate(dateString.substring(0, 10) + " 12:00:00"));

				stats.setNumberOfComments(cursor.getInt(
								  cursor.getColumnIndex(AppStatsTable.KEY_STATS_COMMENTS)));
				stats.setVersionCode(cursor.getInt(
							     cursor.getColumnIndex(AppStatsTable.KEY_STATS_VERSIONCODE)));

				stats.setRating(
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_1STARS)),
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_2STARS)),
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_3STARS)),
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_4STARS)),
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_5STARS)));
				...
				if (!cursor.isNull(idx)) {
					...
					String currency = cursor.getString(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_CURRENCY));
					stats.setTotalRevenue(
						new Revenue(Revenue.Type.TOTAL, amount, currency));
				}
	...
	try {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				stats.setTotalDownloads(cursor.getInt(
								cursor.getColumnIndex(AppStatsTable.KEY_STATS_DOWNLOADS)));
				stats.setDate(Utils.parseDbDate(cursor.getString(
									cursor.getColumnIndex(AppStatsTable.KEY_STATS_REQUESTDATE))));
				stats.setNumberOfComments(cursor.getInt(
								  cursor.getColumnIndex(AppStatsTable.KEY_STATS_COMMENTS)));
				stats.setVersionCode(cursor.getInt(
				...
				stats.setRating(
					...
					cursor.getInt(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_5STARS)));
				int idx =
					cursor.getColumnIndex(AppStatsTable.KEY_STATS_TOTAL_REVENUE);
				if (!cursor.isNull(idx)) {
					double amount = cursor.getDouble(idx);
					String currency = cursor.getString(
						cursor.getColumnIndex(AppStatsTable.KEY_STATS_CURRENCY));
...
public RevenueSummary getRevenueSummaryForApp(final AppInfo app) {
	...
	try {
		...
		int typeIdx =
			cursor.getInt(cursor.getColumnIndex(RevenueSummaryTable.TYPE));
		String currency =
			cursor.getString(cursor.getColumnIndex(RevenueSummaryTable.CURRENCY));
		...
		double lastDay = cursor.getDouble(
			cursor.getColumnIndex(RevenueSummaryTable.LAST_DAY_TOTAL));
		double last7Days = cursor.getDouble(
			cursor.getColumnIndex(RevenueSummaryTable.LAST_7DAYS_TOTAL));
		double last30Days = cursor.getDouble(
			cursor.getColumnIndex(RevenueSummaryTable.LAST_30DAYS_TOTAL));
		double overall = cursor.getDouble(
			cursor.getColumnIndex(RevenueSummaryTable.OVERALL_TOTAL));
		Revenue.Type type = Revenue.Type.values()[typeIdx];
		RevenueSummary result = new RevenueSummary(
			type, currency, date, lastDay, last7Days, last30Days, overall);
...
@SuppressLint("SimpleDateFormat")
public Map<Date, Map<Integer, Integer> >
getDailyRatings(final Date maxDate, final String packagename) {
	Map<Date, Map<Integer, Integer> > result =
		new TreeMap<Date, Map<Integer, Integer> >(new Comparator<Date>() {
			// reverse order
			@Override
	...
	if (maxDate != null) {
		queryString += " and " + AppStatsTable.KEY_STATS_REQUESTDATE +
		               " BETWEEN '" + dateFormatStart.format(new Date()) +
		               "' and '" + dateFormatEnd.format(maxDate) + "'";
	}
	...
	try {
		cursor = context.getContentResolver().query(
			...
			queryString, null, AppStatsTable.KEY_STATS_REQUESTDATE + " asc");
		...
		if (cursor != null && cursor.moveToFirst()) {
			do {
				...
				String dateString = cursor.getString(
					cursor.getColumnIndex(AppStatsTable.KEY_STATS_REQUESTDATE));
				Date date =
					Utils.parseDbDate(dateString.substring(0, 10) + " 00:00:00");
...
public void updateCommentsCache(final List<Comment> comments,
                                final String packageName) {
	...
	for (Comment comment : comments) {
		...
		initialValues.put(CommentsTable.KEY_COMMENT_DATE,
		                  Utils.formatDbDate(comment.getDate()));
		...
		initialValues.put(CommentsTable.KEY_COMMENT_APP_VERSION,
		...
		initialValues.put(CommentsTable.KEY_COMMENT_LANGUAGE,
		                  comment.getLanguage());
		initialValues.put(CommentsTable.KEY_COMMENT_ORIGINAL_TITLE,
		                  comment.getOriginalTitle());
		initialValues.put(CommentsTable.KEY_COMMENT_ORIGINAL_TEXT,
		                  comment.getOriginalText());
		initialValues.put(CommentsTable.KEY_COMMENT_UNIQUE_ID,
		                  comment.getUniqueId());

		context.getContentResolver().insert(CommentsTable.CONTENT_URI,
		                                    initialValues);
	}
...
public List<Comment> getCommentsFromCache(final String packageName) {
	...
	try {
		cursor = context.getContentResolver().query(
			CommentsTable.CONTENT_URI,
			new String[] {
				CommentsTable.KEY_COMMENT_DATE,
				CommentsTable.KEY_COMMENT_PACKAGENAME,
				CommentsTable.KEY_COMMENT_RATING, CommentsTable.KEY_COMMENT_TITLE,
		...
		while (cursor.moveToNext()) {
			...
			String dateString = cursor.getString(
				cursor.getColumnIndex(CommentsTable.KEY_COMMENT_DATE));
			comment.setDate(Utils.parseDbDate(dateString));
			comment.setUser(cursor.getString(
						cursor.getColumnIndex(CommentsTable.KEY_COMMENT_USER)));
			comment.setTitle(cursor.getString(
						 cursor.getColumnIndex(CommentsTable.KEY_COMMENT_TITLE)));
			comment.setText(cursor.getString(
						cursor.getColumnIndex(CommentsTable.KEY_COMMENT_TEXT)));
			comment.setDevice(cursor.getString(
						  cursor.getColumnIndex(CommentsTable.KEY_COMMENT_DEVICE)));
			comment.setAppVersion(cursor.getString(
						      cursor.getColumnIndex(CommentsTable.KEY_COMMENT_APP_VERSION)));
			comment.setRating(cursor.getInt(
						  cursor.getColumnIndex(CommentsTable.KEY_COMMENT_RATING)));
			String replyText = cursor.getString(
				cursor.getColumnIndex(CommentsTable.KEY_COMMENT_REPLY_TEXT));
			if (replyText != null) {
				Comment reply = new Comment(true);
				reply.setText(replyText);
				reply.setDate(Utils.parseDbDate(cursor.getString(
									cursor.getColumnIndex(CommentsTable.KEY_COMMENT_REPLY_DATE))));
...
public long setSkipNotification(final String packageName,
                                final boolean value) {
	ContentValues initialValues = new ContentValues();
	initialValues.put(AppInfoTable.KEY_APP_SKIP_NOTIFICATION,
	                  value == true ? 1 : 0);
