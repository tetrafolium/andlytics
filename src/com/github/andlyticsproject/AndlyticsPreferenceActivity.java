...
public class AndlyticsPreferenceActivity extends PreferenceActivity
...
protected void onCreate(final Bundle savedInstanceState) {
	...
	// Find and setup a listener for auto sync as we have had to adjust the sync
	// handler
	...
	getPreferenceScreen()
	.findPreference(Preferences.DATE_FORMAT_LONG)
...
private void buildAccountsList() {
	developerAccounts =
		DeveloperAccountManager.getInstance(this).getActiveDeveloperAccounts();
...
OnPreferenceClickListener accountPrefrenceClickedListener =
	new OnPreferenceClickListener() {
	@Override
	public boolean onPreferenceClick(final Preference preference) {
		String accountName = (String)preference.getTitle();
		Intent i = new Intent(AndlyticsPreferenceActivity.this,
		                      AccountSpecificPreferenceActivity.class);
...
public boolean onPreferenceChange(final Preference preference,
	...
	if (key.equals(Preferences.AUTOSYNC_PERIOD)) {
		...
		if (!newPeriod.equals(0)) {
			...
			Preferences.saveLastNonZeroAutosyncPeriod(
				AndlyticsPreferenceActivity.this, newPeriod);
		}
		int oldPeriod =
			Preferences.getAutosyncPeriod(AndlyticsPreferenceActivity.this);
		for (DeveloperAccount account : developerAccounts) {
...
protected void onResume() {
	...
	// Make sure we are consistent with any system changes/changes in that
	...
	if (anyEnabled) {
		...
		// the sync period
		autosyncPref.setValue(
			Integer.toString(Preferences.getLastNonZeroAutosyncPeriod(
						 AndlyticsPreferenceActivity.this)));
	} else {
...
// Overkill at the moment, but may be useful in the future as we add more
// options
