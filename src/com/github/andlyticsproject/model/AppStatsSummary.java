...
public class AppStatsSummary extends StatsSummary<AppStats> {
  ...
  public AppStatsSummary() { overallStats = new AppStats(); }
  ...
  @Override
  public void calculateOverallStats(final int limit,
    ...
    if (stats.size() > 1) {
      ...
      for (int currentIndex = 1; currentIndex < stats.size(); currentIndex++) {
        ...
        String olderEntryDate =
            stats.get(currentIndex - 1).getRequestDateString();
        ...
        try {
          ...
          long daysDistance = ((newerDate.getTime() - olderDate.getTime()) /
                               1000 / 60 / 60 / 24);

          for (int i = 1; i < daysDistance; i++) {
            AppStats missingEntry = new AppStats(stats.get(currentIndex - 1));
            missingEntry.setDate(new Date(missingEntry.getDate().getTime() +
                                          (i * 1000 * 60 * 60 * 24)));
            missingAppStats.add(missingEntry);
            missingAppStatsPositionOffest.add(currentIndex +
    ...
    for (int currentIndex = 1; currentIndex < stats.size(); currentIndex++) {
      ...
      if (totalValueDiff == 0 && nullStartIndex < 0) {
        ...
        if (nullStartIndex != -1 && greaterNullDetected && smoothEnabled) {
          ...
          // rounding
          int roundingErrorTotal =
              newerTotalValue -
              (olderTotalValue + ((totalSmoothvalue * (distance))));
          int roundingErrorActive =
              newerActiveValue -
    ...
    for (int i = 0; i < stats.size(); i++) {
      ...
      if (prevStats != null) {
        ...
        stat.setRatingCountDiff(stat.getRatingCount() -
                                prevStats.getRatingCount());
    ...
    if (stats.size() > 0) {
      ...
      overallStats.setActiveInstalls(last.getActiveInstalls() -
                                     first.getActiveInstalls());
      overallStats.setTotalDownloads(last.getTotalDownloads() -
                                     first.getTotalDownloads());
      ...
      overallStats.init();
      overallStats.setDailyDownloads(
          (last.getTotalDownloads() - first.getTotalDownloads()) /
          stats.size());

      BigDecimal avgBigDecimal =
          new BigDecimal(overallAvgRating / stats.size());
      ...
      BigDecimal percentBigDecimal =
          new BigDecimal(overallActiveInstallPercent / stats.size());
      percentBigDecimal =
          percentBigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP);

      overallStats.setActiveInstallsPercentString(
          percentBigDecimal.toPlainString() + "");
      ...
      if (currency != null) {
        overallStats.setTotalRevenue(
            new Revenue(Revenue.Type.TOTAL, totalRevenue, currency));
      }
    }
  }
  ...
  public Integer getLowestRatingChange() { return lowestRatingChange; }

  public void setHighestRatingChange(final Integer highestRatingChange) {
    this.highestRatingChange = highestRatingChange;
  }

  public Integer getHighestRatingChange() { return highestRatingChange; }
}
