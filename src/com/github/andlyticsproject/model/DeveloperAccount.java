...
public class DeveloperAccount {
  ...
  public enum State { HIDDEN, ACTIVE, SELECTED }
  ;
  ...
  public void setLastStatsUpdate(final Date lastStatsUpdate) {
    this.lastStatsUpdate =
        lastStatsUpdate == null ? null : (Date)lastStatsUpdate.clone();
  }
  ...
  public String toString() {
    return String.format("DeveloperAccount [name=%s, developerId=%s, state=%s]",
                         name, developerId, state);
  }
  ...
  public boolean isSelected() { return state == State.SELECTED; }

  public boolean isVisible() { return state != State.HIDDEN; }

  public void select() { state = State.SELECTED; }
