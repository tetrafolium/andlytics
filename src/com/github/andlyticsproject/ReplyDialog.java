...
public View onCreateView(final LayoutInflater inflater,
	...
	final EditText replyText =
		(EditText)view.findViewById(R.id.comment_reply_dialog_text);
	final TextView comment_reply_dialog_counter =
		(TextView)view.findViewById(R.id.comment_reply_dialog_counter);
	final TextView comment_reply_dialog_max_characters =
		(TextView)view.findViewById(R.id.comment_reply_dialog_max_characters);
	...
	replyText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(final CharSequence reply, final int start,
			        ...
			        comment_reply_dialog_counter.setText(String.valueOf(reply.length()));

			        // enable/disable button and add color-coding
			        if (ReplyDialog.DEVELOPER_REPLY_MAX_CHARACTERS < reply.length()) {
			                okButton.setEnabled(false);
			                comment_reply_dialog_counter.setTextColor(Color.RED);
			                comment_reply_dialog_max_characters.setTextColor(Color.RED);
				} else if (1 > reply.length()) {
			                okButton.setEnabled(false);
				} else {
			                okButton.setEnabled(true);
			                comment_reply_dialog_counter.setTextColor(
						getResources().getColor(R.color.greyText));
			                comment_reply_dialog_max_characters.setTextColor(
						getResources().getColor(R.color.greyText));
				}
			}
