...
public class AdmobAuthenticationUtilities {
  ...
  private static final String TAG =
      AdmobAuthenticationUtilities.class.getSimpleName();
  ...
  public static String authenticate(final String username,
    ...
    try {
      ...
      Log.e(
          TAG,
          "Unsuccessful Admob authentication, google accounts are not supported");
   ...
   */
  private static void sendResult(final String result, final Handler handler,
                                 final Context context) {
  ...
  public static Thread attemptAuth(final String username, final String password,
    ...
    final Runnable runnable = new Runnable() {
      public void run() { authenticate(username, password, handler, context); }
    };
    // run on background thread.
    return AdmobAuthenticationUtilities.performOnBackgroundThread(runnable);
  }

  public static String authenticateAccount(final String accountName,
                                           final Context context) {
  ...
  public static void invalidateToken(final String token,
    ...
    AccountManager manager = AccountManager.get(context);
    manager.invalidateAuthToken(AdmobAccountAuthenticator.ACCOUNT_TYPE_ADMOB,
                                token);
  }
}
