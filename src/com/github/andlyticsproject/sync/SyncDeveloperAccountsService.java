...
private static final String TAG =
	SyncDeveloperAccountsService.class.getSimpleName();
...
protected void onHandleIntent(final Intent intent) {
	...
	Log.d(TAG,
	      "Synchornizing Andlytics developer accounts and Google accounts...");
...
private void addNewGoogleAccounts(final Account[] googleAccounts) {
	...
	for (Account googleAccount : googleAccounts) {
		DeveloperAccount account =
			DeveloperAccount.createHidden(googleAccount.name);
...
private void syncData() {
	List<DeveloperAccount> accounts =
		developerAccountManager.getActiveDeveloperAccounts();
	for (DeveloperAccount developerAccount : accounts) {
		...
		boolean syncAutomatically = ContentResolver.getSyncAutomatically(
			googleAccount, AutosyncHandler.ACCOUNT_AUTHORITY);
		if (syncAutomatically && developerAccount.isVisible()) {
			...
			ContentResolver.requestSync(googleAccount,
...
private Account findMatchingAccount(final DeveloperAccount account,
