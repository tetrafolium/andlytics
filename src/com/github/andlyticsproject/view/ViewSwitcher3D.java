...
/**
 * This code was extracted from the Transition3D sample activity found in the
 * Android ApiDemos. The animation is made of two smaller animations: the first
 * half rotates the list by 90 degrees on the Y axis and the second half rotates
...
public class ViewSwitcher3D {
  ...
  public void setDuration(final long duration) { mDuration = duration; }
  ...
  public void swap() {
    ...
    Rotate3dAnimation rotation = new Rotate3dAnimation(
        start, end, mContainer.getWidth() / 2.0f, mContainer.getHeight() / 2.0f,
        mDepthOfRotation, true, getListener());
  ...
  /**
   * Listen for the end of the first half of the animation. Then post a new
   * action that effectively swaps the views when the container is rotated 90
   * degrees and thus invisible.
   */
  private final class TurnAroundListener
    ...
    public void run() {
      ...
      if (isFrontsideVisible()) {
        ...
        rotation = new Rotate3dAnimation(
            90, 180, centerX, centerY, mDepthOfRotation, false, getListener());
      } else {
        ...
        rotation = new Rotate3dAnimation(
            90, 0, centerX, centerY, mDepthOfRotation, false, getListener());
      }
  ...
  private void unmirrorTheBackside() {
    Rotate3dAnimation rotation = new Rotate3dAnimation(
        0, 180, mContainer.getWidth() / 2.0f, mContainer.getHeight() / 2.0f,
        mDepthOfRotation, false, getListener());
  ...
  public ViewSwitcherListener getListener() { return listener; }
