...
public static void foregroundSyncStats(final Context context,
...
private static AdSense createForegroundSyncClient(final Context context,
                                                  final String admobAccount) {
	GoogleAccountCredential credential = GoogleAccountCredential.usingOAuth2(
		context, Collections.singleton(AdSenseScopes.ADSENSE_READONLY));
...
public static void
backgroundSyncStats(final Context context, final String admobAccount,
                    final List<String> adUnits, final Bundle extras,
                    final String authority, final Bundle syncBundle)
throws Exception {
	try {
		AdSense adsense = createBackgroundSyncClient(
			context, admobAccount, extras, authority, syncBundle);
		syncStats(context, adsense, adUnits);
	} catch (GoogleJsonResponseException e) {
		List<ErrorInfo> errors = e.getDetails().getErrors();
		for (ErrorInfo err : errors) {
...
private static void syncStats(final Context context, final AdSense adsense,
	...
	List<AdmobStats> result =
		generateReport(adsense, account, adClientId, startDate, endDate);
...
private static Calendar[] getSyncPeriod(final List<String> adUnits) {
	...
	for (String adUnit : adUnits) {
		...
		List<AdmobStats> admobStats =
			contentAdapter.getAdmobStats(adUnit, Timeframe.LATEST_VALUE)
			.getStats();
...
private static String getClientId(final AdSense adsense,
                                  final Account account) throws IOException {
	AdClients adClients = adsense.accounts()
	                      .adclients()
	                      .list(account.getId())
...
private static Account getFirstAccount(final AdSense adsense)
throws IOException {
...
private static void updateStats(final Context context,
                                final boolean bulkInsert,
                                final List<AdmobStats> result) {
	ContentAdapter contentAdapter = ContentAdapter.getInstance(
...
private static AdSense createBackgroundSyncClient(final Context context,
                                                  final String admobAccount,
                                                  final Bundle extras,
                                                  final String authority,
                                                  final Bundle syncBundle) {
	BackgroundGoogleAccountCredential credential =
		BackgroundGoogleAccountCredential.usingOAuth2(
			context, Collections.singleton(AdSenseScopes.ADSENSE_READONLY),
...
private static List<AdmobStats>
generateReport(final AdSense adsense, final Account account,
               final String adClientId, final Date startDate,
	...
	Generate request = adsense.accounts().reports().generate(
		account.getId(), startDateStr, endDateStr);

	// Specify the desired ad client using a filter.
	request.setFilter(
		Arrays.asList("AD_CLIENT_ID==" + escapeFilterParameter(adClientId)));

	request.setDimension(
		Arrays.asList("DATE", "AD_UNIT_ID", "AD_UNIT_CODE", "AD_UNIT_NAME"));
	request.setMetric(Arrays.asList(
	...
	if (DEBUG) {
		...
		for (AdsenseReportsGenerateResponse.Headers header :
		     response.getHeaders()) {
	...
	AdsenseReportsGenerateResponse.Headers revenueHeader =
		response.getHeaders().get(11);
...
public static Map<String, String> getAdUnits(final Context context,
                                             final String admobAccount)
throws IOException {
	AdSense adsense = createForegroundSyncClient(context, admobAccount);
	...
	AdUnits units = adsense.accounts()
	                .adunits()
	                .list(account.getId(), adClientId)
	                .setMaxResults(MAX_LIST_PAGE_SIZE)
	                .setPageToken(null)
	                .execute();
