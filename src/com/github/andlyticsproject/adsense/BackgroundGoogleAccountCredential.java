 ...
 *
 * <p>
 * When fetching a token, any thrown {@link GoogleAuthException} would be
 * wrapped: <ul> <li>{@link GooglePlayServicesAvailabilityException} would be
 * wrapped inside of
 ...
 * <li>{@link GoogleAuthException} when be wrapped inside of {@link
 * GoogleAuthIOException}</li>
 * </ul>
 * </p>
 *
 * <p>
 * Upgrade warning: in prior version 1.14 exponential back-off was enabled by
 * default when I/O exception was thrown inside {@link #getToken}, but starting
 * with version 1.15 you need to call
 ...
 */
@SuppressLint("NewApi")
public class BackgroundGoogleAccountCredential
  ...
  /**
   * Selected Google account name (e-mail address), for example {@code
   * "johndoe@gmail.com"}, or
   * {@code null} for none.
   */
  ...
  /**
   * Back-off policy which is used when an I/O exception is thrown inside {@link
   * #getToken} or
   * {@code null} for none.
   */
   ...
   * scope to use on {@link GoogleAuthUtil#getToken}
   */
  public BackgroundGoogleAccountCredential(final Context context,
                                           final String scope,
                                           final Bundle extras,
   ...
   *     #usingOAuth2(Context, Collection)}
   * instead.
   */
  //  @Deprecated
  //  public static GoogleAccountCredential usingOAuth2(Context context, String
  //  scope,
  //                  String... extraScopes) {
  //          StringBuilder scopeBuilder = new
  //          StringBuilder("oauth2:").append(scope); for (String extraScope :
  //          extraScopes) {
  //                  scopeBuilder.append(' ').append(extraScope);
  //          }
  //          return new GoogleAccountCredential(context,
   ...
   * @since 1.15
   */
  public static BackgroundGoogleAccountCredential
  usingOAuth2(final Context context, final Collection<String> scopes,
              final Bundle extras, final String authority,
              final Bundle syncBundle) {
    Preconditions.checkArgument(scopes != null && scopes.iterator().hasNext());
    String scopesStr = "oauth2: " + Joiner.on(' ').join(scopes);

    return new BackgroundGoogleAccountCredential(context, scopesStr, extras,
   ...
   */
  public static BackgroundGoogleAccountCredential
  usingAudience(final Context context, final String audience,
                final Bundle extras, final String authority,
                final Bundle syncBundle) {
    Preconditions.checkArgument(audience.length() != 0);

    return new BackgroundGoogleAccountCredential(
        context, "audience:" + audience, extras, authority, syncBundle);
  }
   ...
   */
  public final BackgroundGoogleAccountCredential
  setSelectedAccountName(final String accountName) {
  ...
  public final String getScope() { return scope; }
  ...
  public final Account getSelectedAccount() { return selectedAccount; }
  ...
  /**
   * Sets the back-off policy which is used when an I/O exception is thrown
   * inside
   * {@link #getToken} or {@code null} for none.
   *
   * @since 1.15
   ...
   * @since 1.15
   */
  public final Sleeper getSleeper() { return sleeper; }
   ...
   * {@code "johndoe@gmail.com"}, or {@code null} for none.
   */
  public final String getSelectedAccountName() { return accountName; }

  /**
   * Returns an intent to show the user to select a Google account, or create a
   * new one if there are none on the device yet.
   *
   * <p>
  ...
  public final Intent newChooseAccountIntent() {
    return AccountPicker.newChooseAccountIntent(
        selectedAccount, null, new String[] {GoogleAccountManager.ACCOUNT_TYPE},
