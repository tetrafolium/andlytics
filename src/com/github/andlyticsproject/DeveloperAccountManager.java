...
public class DeveloperAccountManager {
  ...
  private static final String TAG =
      DeveloperAccountManager.class.getSimpleName();

  private static DeveloperAccountManager instance;

  private AndlyticsDb andlyticsDb;

  public static synchronized DeveloperAccountManager
  ...
  addOrUpdateDeveloperAccount(final DeveloperAccount account) {
  ...
  public List<DeveloperAccount> getAllDeveloperAccounts() {
    ...
    try {
      c = db.query(DeveloperAccountsTable.DATABASE_TABLE_NAME,
                   DeveloperAccountsTable.ALL_COLUMNS, null, null, null, null,
                   "_id asc", null);
      while (c.moveToNext()) {
  ...
  public List<DeveloperAccount> getActiveDeveloperAccounts() {
    ...
    try {
      c = db.query(
          ...
          new String[] {
              Integer.toString(DeveloperAccount.State.ACTIVE.ordinal()),
  ...
  public DeveloperAccount getSelectedDeveloperAccount() {
    List<DeveloperAccount> accounts =
        getDeveloperAccountsByState(DeveloperAccount.State.SELECTED);
    if (accounts.isEmpty()) {
      return null;
    }
    ...
    if (accounts.size() > 1) {
      throw new IllegalStateException(
          "More than one selected developer account: " + accounts);
    }

    return accounts.get(0);
  }

  public List<DeveloperAccount>
  getDeveloperAccountsByState(final DeveloperAccount.State state) {
    List<DeveloperAccount> result = new ArrayList<DeveloperAccount>();
    ...
    try {
      c = db.query(DeveloperAccountsTable.DATABASE_TABLE_NAME,
                   ...
                   new String[] {Integer.toString(state.ordinal())}, null, null,
                   "_id asc", null);
      while (c.moveToNext()) {
        DeveloperAccount account = createAcount(c);
        result.add(account);
  ...
  public DeveloperAccount findDeveloperAccountById(final long id) {
    ...
    try {
      c = db.query(DeveloperAccountsTable.DATABASE_TABLE_NAME,
                   ...
                   new String[] {Long.toString(id)}, null, null, "_id asc",
                   null);
      if (!c.moveToNext()) {
        return null;
      }
  ...
  public DeveloperAccount findDeveloperAccountByName(final String name) {
    ...
    try {
      c = db.query(DeveloperAccountsTable.DATABASE_TABLE_NAME,
                   DeveloperAccountsTable.ALL_COLUMNS, "name = ?",
                   new String[] {name}, null, null, "_id asc", null);
      if (!c.moveToNext()) {
        return null;
      }
  ...
  updateDeveloperAccount(final DeveloperAccount account) {
  ...
  public synchronized void
  deleteDeveloperAccount(final DeveloperAccount account) {
    SQLiteDatabase db = andlyticsDb.getWritableDatabase();
  ...
  public synchronized void selectDeveloperAccount(final String name) {
    ...
    db.beginTransaction();
    try {
      List<DeveloperAccount> currentlySelected =
          getDeveloperAccountsByState(DeveloperAccount.State.SELECTED);
  ...
  public synchronized void unselectDeveloperAccount() {
    ...
    values.put(DeveloperAccountsTable.STATE,
               DeveloperAccount.State.ACTIVE.ordinal());
    db.update(DeveloperAccountsTable.DATABASE_TABLE_NAME, values,
              DeveloperAccountsTable.STATE + " = ?",
              new String[] {
                  Integer.toString(DeveloperAccount.State.SELECTED.ordinal())});
  }
  ...
  private void setAccountState(final String accountName,
                               final DeveloperAccount.State state) {
    SQLiteDatabase db = andlyticsDb.getWritableDatabase();
    ContentValues values = new ContentValues();
    values.put(DeveloperAccountsTable.STATE, state.ordinal());
    db.update(DeveloperAccountsTable.DATABASE_TABLE_NAME, values,
              DeveloperAccountsTable.NAME + " = ?", new String[] {accountName});
  }
  ...
  private DeveloperAccount createAcount(final Cursor c) {
    ...
    DeveloperAccount.State currentState =
        DeveloperAccount.State
            .values()[c.getInt(c.getColumnIndex(DeveloperAccountsTable.STATE))];
    Date lastStatsUpdate = new Date(
        c.getLong(c.getColumnIndex(DeveloperAccountsTable.LAST_STATS_UPDATE)));
  ...
  public synchronized void
  saveLastStatsRemoteUpdateTime(final String accountName,
