...
public class DetailsActivity extends BaseActivity
	implements DetailedStatsActivity, CommentReplier, ChartSwitcher {

private static final String TAG = DetailsActivity.class.getSimpleName();

public static final String EXTRA_CHART_SET =
	"com.github.andlyticsproject.chartset";
public static final String EXTRA_CHART_NAME =
	"com.github.andlyticsproject.chartname";

private static final String REPLY_DIALOG_FRAGMENT = "reply_dialog_fragment";

private static final String[] TAB_TAGS = {"comments_tab", "ratings_tab",
	                                  "downloads_tab", "revenue_tab",
	                                  "admob_tab"};
...
public static String EXTRA_HAS_REVENUE =
	"com.github.andlyticsproject.hasRevenue";

private String appName;
private boolean hasRevenue;

public static class TabListener<T extends StatsView<?> >
...
public TabListener(final DetailsActivity activity, final String tag,
                   final Class<T> clz) {
...
protected void onCreate(final Bundle savedInstanceState) {
	...
	Tab tab = actionBar.newTab()
	          ...
	          .setTabListener(new TabListener<CommentsFragment>(
					  this, "comments_tab", CommentsFragment.class));
	actionBar.addTab(tab);

	tab = actionBar.newTab()
	      .setText(R.string.ratings)
	      .setTabListener(new TabListener<RatingsFragment>(
				      this, "ratings_tab", RatingsFragment.class));
	actionBar.addTab(tab);

	tab = actionBar.newTab()
	      .setText(R.string.downloads)
	      .setTabListener(new TabListener<DownloadsFragment>(
				      this, "downloads_tab", DownloadsFragment.class));
	actionBar.addTab(tab);

	if (hasRevenue) {
		tab = actionBar.newTab()
		      .setText(R.string.revenue)
		      .setTabListener(new TabListener<RevenueFragment>(
					      this, "revenue_tab", RevenueFragment.class));
		actionBar.addTab(tab);
	}

	// Check if AdMob is configured for this app
	String[] admobDetails =
		AndlyticsDb.getInstance(this).getAdmobDetails(packageName);
	...
	if (admobConfigured || !Preferences.getHideAdmobForUnconfiguredApps(this)) {
		tab = actionBar.newTab()
		      ...
		      .setTabListener(new TabListener<AdmobFragment>(
					      this, "admob_tab", AdmobFragment.class));
		actionBar.addTab(tab);
	}

	int selectedTabIdx =
		getIntent().getExtras().getInt(EXTRA_SELECTED_TAB_IDX, 0);
	// FIXME This is a hack to select AdMob in the case that the revenue tab
	// isn't enabled
...
protected void onSaveInstanceState(final Bundle state) {
	...
	state.putInt(EXTRA_SELECTED_TAB_IDX,
...
public void showReplyDialog(final Comment comment) {
	...
	Fragment prev =
		getSupportFragmentManager().findFragmentByTag(REPLY_DIALOG_FRAGMENT);
	if (prev != null) {
		ft.remove(prev);
	}
	...
	args.putString(ReplyDialog.ARG_REPLY, comment.getReply() == null
	                                      ? ""
...
public void replyToComment(final String commentUniqueId,
	...
	Utils.execute(new DetachableAsyncTask<Void, Void, Comment, DetailsActivity>(
			      this) {
			Exception error;
			...
			protected Comment doInBackground(final Void ... arg0) {
			        ...
			        try {
			                DevConsoleV2 devConsole =
						DevConsoleRegistry.getInstance().get(accountName);

			                return devConsole.replyToComment(DetailsActivity.this, packageName,
			                                                 developerId, commentUniqueId,
			                                                 replyText);
				} catch (Exception e) {
			                error = e;
			                return null;
			...
			protected void onPostExecute(final Comment reply) {
			        ...
			        CommentsFragment commentsFargment =
					(CommentsFragment)getSupportFragmentManager().findFragmentByTag(
						"comments_tab");
...
public void setCurrentChart(final int currentPage, final int column) {
	String tabTag =
		TAB_TAGS[getSupportActionBar().getSelectedNavigationIndex()];
	StatsView<?> chartFargment =
		(StatsView<?>)getSupportFragmentManager().findFragmentByTag(tabTag);
...
protected void onActivityResult(final int requestCode, final int resultCode,
	...
	if (requestCode == REQUEST_GOOGLE_PLAY_SERVICES) {
		...
		if (resultCode == Activity.RESULT_OK) {
			if (getSupportActionBar().getSelectedNavigationIndex() ==
			    TAB_IDX_ADMOB) {
				String tabTag =
					TAB_TAGS[getSupportActionBar().getSelectedNavigationIndex()];
				AdmobFragment admobFragment =
					(AdmobFragment)getSupportFragmentManager().findFragmentByTag(
						tabTag);
