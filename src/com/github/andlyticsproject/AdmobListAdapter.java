...
public void notifyDataSetChanged() {
	this.dateFormat =
		new SimpleDateFormat(Preferences.getDateFormatStringShort(activity));
...
throws IndexOutOfBoundsException {
	...
	case 0: {
		switch (column) {
		case COL_REVENUE:
			tv.setText(getNumberFormat(admob.getCurrencyCode())
			           .format(admob.getRevenue()));
			return;
		case COL_EPC:
			//                              tv.setText(admob.getEpcCents());
			// tv.setText(getNumberFormat(admob.getCurrencyCode()).format(admob.getEpc()));
			tv.setText(getNumberFormat(admob.getCurrencyCode())
			           .format(admob.getCpcRevenue()));
			return;
		case COL_REQUESTS:
	...
	case 1: {
		...
		case COL_ECPM:
			tv.setText(
				getNumberFormat(admob.getCurrencyCode()).format(admob.getEcpm()));
			return;
		case COL_IMPRESSIONS:
...
public abstract class AdmobValueCallbackHander
	implements ValueCallbackHander {
@Override
...
@Override
public boolean isHeilightValue(final Object current,
                               final Object previouse) {
...
throws IndexOutOfBoundsException {
	...
	String currencyCode =
		stats.isEmpty() ? "USD" : stats.get(0).getCurrencyCode();
	switch (page) {
	case 0: {
		switch (column) {
		case COL_REVENUE:
			return (overallStats != null)
	    ? getNumberFormat(currencyCode).format(overallStats.getRevenue())
	    : "";

		case COL_EPC:
			return (overallStats != null)
	    ? getNumberFormat(currencyCode).format(overallStats.getEpc())
	    : "";
		...
		case COL_FILL_RATE:
			return (overallStats != null)
	    ? (new BigDecimal(overallStats.getFillRate() * 100))
	...
	case 1: {
		...
		case COL_ECPM:
			return (overallStats != null)
	    ? getNumberFormat(currencyCode).format(overallStats.getEcpm())
	    : "";
		...
		case COL_CTR:
			return (overallStats != null)
	    ? (new BigDecimal(overallStats.getCtr() * 100))
			       .setScale(2, BigDecimal.ROUND_HALF_UP)
			       .toPlainString() +
			       "%"
	    : "";
...
private NumberFormat US_NUMBER_FORMAT =
	NumberFormat.getCurrencyInstance(Locale.US);
private Map<String, NumberFormat> currencyFormats =
