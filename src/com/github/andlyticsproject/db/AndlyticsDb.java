...
public class AndlyticsDb extends SQLiteOpenHelper {
  ...
  public void onUpgrade(final SQLiteDatabase db, final int oldVersion,
    ...
    if (oldVersion < 19) {
      ...
      db.execSQL("DROP TABLE IF EXISTS " +
                 DeveloperAccountsTable.DATABASE_TABLE_NAME);
      db.execSQL(DeveloperAccountsTable.TABLE_CREATE_DEVELOPER_ACCOUNT);
    ...
    if (oldVersion < 20) {
      ...
      //                  DeveloperAccountsTable.DATABASE_TABLE_NAME + " add "
      //                                  + DeveloperAccountsTable.DEVELOPER_ID
      //                                  + " text");
    }
    if (oldVersion < 21) {
      Log.w(TAG, "Old version < 21 - adding revenue_summary table");
      db.execSQL("DROP TABLE IF EXISTS " +
                 RevenueSummaryTable.DATABASE_TABLE_NAME);
      db.execSQL(RevenueSummaryTable.TABLE_CREATE_REVENUE_SUMMARY);
    ...
    if (oldVersion == 21) {
      ...
      db.execSQL("ALTER table " + RevenueSummaryTable.DATABASE_TABLE_NAME +
                 " add " + RevenueSummaryTable.DATE + " date");
  ...
  private void migrateAppInfoPrefs(final SQLiteDatabase db) {
    ...
    try {
      ...
      for (Package p : packages) {
        ...
        if (admobSiteId != null) {
          String admobAccount =
              Preferences.getAdmobAccount(context, admobSiteId);
        ...
        long lastCommentsUpdate =
            Preferences.getLastCommentsRemoteUpdateTime(context, p.name);
        if (lastCommentsUpdate != 0) {
          ContentValues values = new ContentValues();
          values.put(AppInfoTable.KEY_APP_LAST_COMMENTS_UPDATE,
                     lastCommentsUpdate);
          db.update(AppInfoTable.DATABASE_TABLE_NAME, values, "_id = ?",
                    new String[] {Long.toString(p.id)});
        }
    ...
    Log.d(TAG, String.format(
                   "Successfully migrated app info settings for %d packages",
                   migrated));
  }
  ...
  private void migrateAccountsFromPrefs(final SQLiteDatabase db) {
    ...
    try {
      ...
      Account[] accounts =
          am.getAccountsByType(AutosyncHandler.ACCOUNT_TYPE_GOOGLE);
      String activeAccount = Preferences.getAccountName(context);
      for (Account account : accounts) {
        boolean isHidden =
            Preferences.getIsHiddenAccount(context, account.name);
        long lastStatsUpdate =
            Preferences.getLastStatsRemoteUpdateTime(context, account.name);
        ...
        DeveloperAccount developerAccount =
            new DeveloperAccount(account.name, state);
    ...
    Log.d(TAG, String.format("Successfully migrated %d developer accounts",
                             migrated));
  }

  private long addDeveloperAccount(final SQLiteDatabase db,
                                   final DeveloperAccount account) {
    ContentValues values = toValues(account);

    return db.insertOrThrow(DeveloperAccountsTable.DATABASE_TABLE_NAME, null,
                            values);
  }
  ...
  public static ContentValues toValues(final DeveloperAccount account) {
    ...
    long updateTime = account.getLastStatsUpdate() == null
                          ? 0
                          : account.getLastStatsUpdate().getTime();
    result.put(DeveloperAccountsTable.LAST_STATS_UPDATE, updateTime);
    // XXX
    //              result.put(DeveloperAccountsTable.DEVELOPER_ID,
  ...
  public String[] getAdmobDetails(final String packageName) {
    ...
    try {
      c = db.query(AppInfoTable.DATABASE_TABLE_NAME,
                   new String[] {AppInfoTable.KEY_APP_ADMOB_ACCOUNT,
      ...
      result[0] =
          c.getString(c.getColumnIndex(AppInfoTable.KEY_APP_ADMOB_ACCOUNT));
      result[1] =
          c.getString(c.getColumnIndex(AppInfoTable.KEY_APP_ADMOB_SITE_ID));
      result[2] =
          c.getString(c.getColumnIndex(AppInfoTable.KEY_APP_ADMOB_AD_UNIT_ID));
  ...
  public synchronized void saveAdmobDetails(final String packageName,
                                            final String admobAccount,
  ...
  public synchronized void saveAdmobAdUnitId(final String packageName,
                                             final String admobAccount,
                                             final String admobAdUnitId) {
    SQLiteDatabase db = getWritableDatabase();
    db.beginTransaction();
  ...
  getLastCommentsRemoteUpdateTime(final String packageName) {
    ...
    try {
      c = db.query(AppInfoTable.DATABASE_TABLE_NAME,
                   ...
                   new String[] {packageName}, null, null, null);
      if (c.getCount() != 1) {
        Log.w(TAG, String.format("Unexpected package count for %s: %d",
                                 packageName, c.getCount()));
      }
      if (c.getCount() < 1 || !c.moveToNext()) {
        throw new IllegalStateException(String.format(
            "Package name not found in AppInfo table: %s. count=%d",
            packageName, c.getCount()));
      }
  ...
  saveLastCommentsRemoteUpdateTime(final String packageName,
  ...
  private long findPackageId(final SQLiteDatabase db,
  ...
  public synchronized AppInfo findAppByPackageName(final String packageName) {
    ...
    try {
      ...
      appInfo.setAccount(cursor.getString(
          cursor.getColumnIndex(AppInfoTable.KEY_APP_ACCOUNT)));
      ...
      appInfo.setGhost(
          cursor.getInt(cursor.getColumnIndex(AppInfoTable.KEY_APP_GHOST)) == 0
              ? false
              : true);
      ...
      appInfo.setRatingDetailsExpanded(
          cursor.getInt(
              cursor.getColumnIndex(AppInfoTable.KEY_APP_RATINGS_EXPANDED)) == 0
              ? false
              : true);
  ...
  public synchronized void fetchAppDetails(final AppInfo appInfo) {
    ...
    try {
      c = db.query(
          AppDetailsTable.DATABASE_TABLE_NAME, AppDetailsTable.ALL_COLUMNS,
      ...
      String description =
          c.getString(c.getColumnIndex(AppDetailsTable.DESCRIPTION));
  ...
  public synchronized void updateAppDetails(final AppDetails details) {
    ...
    long updateTime = details.getLastStoreUpdate() == null
                          ? 0
                          : details.getLastStoreUpdate().getTime();
    values.put(AppDetailsTable.LAST_STORE_UPDATE, updateTime);

    getWritableDatabase().update(AppDetailsTable.DATABASE_TABLE_NAME, values,
                                 "_id = ?",
  ...
  public synchronized void insertOrUpdateAppDetails(final AppInfo appInfo) {
    ...
    try {
      c = db.query(AppDetailsTable.DATABASE_TABLE_NAME,
                   new String[] {AppDetailsTable.ROWID},
                   AppDetailsTable.APPINFO_ID + "=?",
  ...
  public synchronized ArrayList<Link> getLinksForApp(final long appDetailsId) {
    ...
    try {
      cursor = db.query(LinksTable.DATABASE_TABLE_NAME, LinksTable.ALL_COLUMNS,
                        ...
                        new String[] {Long.toString(appDetailsId)}, null, null,
                        LinksTable.ROWID);
      if (cursor == null) {
        return result;
      }
      ...
      while (cursor.moveToNext()) {
        ...
        link.setName(
            cursor.getString(cursor.getColumnIndex(LinksTable.LINK_NAME)));
        link.setURL(
            cursor.getString(cursor.getColumnIndex(LinksTable.LINK_URL)));
  ...
  public synchronized void deleteLink(final long id) {
    getWritableDatabase().delete(LinksTable.DATABASE_TABLE_NAME,
                                 LinksTable.ROWID + "=?",
                                 new String[] {Long.toString(id)});
  }

  public synchronized void addLink(final AppDetails appDetails,
    ...
    getWritableDatabase().insertOrThrow(LinksTable.DATABASE_TABLE_NAME, null,
                                        values);
  }

  public synchronized void editLink(final Long id, final String url,
                                    final String name) {
    ContentValues values = new ContentValues();
    values.put(LinksTable.LINK_URL, url);
    values.put(LinksTable.LINK_NAME, name);
  ...
  public synchronized void fetchRevenueSummary(final AppInfo appInfo) {
    ...
    Cursor c = null;
    try {
      c = db.query(RevenueSummaryTable.DATABASE_TABLE_NAME,
      ...
      String currency =
          c.getString(c.getColumnIndex(RevenueSummaryTable.CURRENCY));
      ...
      double lastDayTotal =
          c.getDouble(c.getColumnIndex(RevenueSummaryTable.LAST_DAY_TOTAL));
      double last7DaysTotal =
          c.getDouble(c.getColumnIndex(RevenueSummaryTable.LAST_7DAYS_TOTAL));
      double last30DaysTotal =
          c.getDouble(c.getColumnIndex(RevenueSummaryTable.LAST_30DAYS_TOTAL));
      double overallTotal =
          c.getDouble(c.getColumnIndex(RevenueSummaryTable.OVERALL_TOTAL));

      Revenue.Type type = Revenue.Type.values()[typeIdx];
      RevenueSummary revenue =
          new RevenueSummary(type, currency, date, lastDayTotal, last7DaysTotal,
                             last30DaysTotal, overallTotal);
      revenue.setId(id);
