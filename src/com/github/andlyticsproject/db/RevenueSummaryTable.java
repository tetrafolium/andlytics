...
public class RevenueSummaryTable {
  ...
  public static final String TABLE_CREATE_REVENUE_SUMMARY =
      "create table " + DATABASE_TABLE_NAME +
      " (_id integer primary key autoincrement, " + TYPE +
      " integer not null, " + CURRENCY + " text not null, " + DATE + " date, " +
      LAST_DAY_TOTAL + " double not null, " + LAST_7DAYS_TOTAL +
      " double not null, " + LAST_30DAYS_TOTAL + " double not null, " +
      OVERALL_TOTAL + " double not null, " + APPINFO_ID +
      " integer not null, foreign key(appinfo_id) references appinfo(_id))";

  public static final String[] ALL_COLUMNS = {ROWID,
                                              TYPE,
                                              CURRENCY,
                                              DATE,
                                              LAST_DAY_TOTAL,
  ...
  static {
    ...
    PROJECTION_MAP.put(RevenueSummaryTable.CURRENCY,
                       RevenueSummaryTable.CURRENCY);
    PROJECTION_MAP.put(RevenueSummaryTable.DATE, RevenueSummaryTable.DATE);
    PROJECTION_MAP.put(RevenueSummaryTable.LAST_DAY_TOTAL,
                       RevenueSummaryTable.LAST_DAY_TOTAL);
    ...
    PROJECTION_MAP.put(RevenueSummaryTable.APPINFO_ID,
